...
def evaluate(name, cat, n, dataset, language):
	compl_dict=json.load(open(name,"r"))
	df = pd.read_csv(dataset,  encoding='utf-8', sep='\t')
	...
	for i in compl_dict.keys():
		...
		if language == 'fr':
			cats = [w.lower() for w in fr_target_dict[cat]]
		else:
			cats = [w.lower() for w in eng_target_dict[cat]]
		df = df.replace('\n','', regex=True)
...
def study_results_at_per_cat(m, cat, print_at, language, dataset):
	names = {'mBERT': 'bert-base-multilingual-uncased', 'XLMR-B': 'xlm-roberta-base', 'XLMR-L': 'xlm-roberta-large'}
	recall = evaluate('mlm_output/'+names[m]+'_'+language+'-corrected.json', cat, print_at, dataset, language)
