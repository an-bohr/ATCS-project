...
# Import files
sys.path.append("..")
from utils.target_dicts_fr import fr_target_dict, templates, country_templates
...
def google(keywords, query, temp, group, cat):
	...
	'''
	url = "http://suggestqueries.google.com/complete/search?output=chrome&q=" + keywords + "&hl=fr"
	ua = UserAgent()
...
def yahoo(keywords, query, temp, group, cat):
	...
	'''
	url = 'http://sugg.search.yahoo.net/sg/?output=chromep&nresults=10&command=' + keywords + "&hl=fr"
	ua = UserAgent()
	headers = {"user-agent": ua.chrome}
	response = requests.get(url, headers=headers, verify=False)
	suggestions = json.loads(response.text)
...
def duckduckgo(keywords, query, temp, group, cat):
	...
	'''
	url = 'https://duckduckgo.com/ac/?q=' + keywords + "&hl=fr"
	ua = UserAgent()
	headers = {"user-agent": ua.chrome}
	response = requests.get(url, headers=headers, verify=False)
	suggestions = json.loads(response.text)
...
def merge_data(engine1, engine2, engine3, engine4, savefile):
	...
	'''
	df1 = pd.read_csv('stereo_dataset/'+engine1+'_data_fr.csv', encoding='utf-8', sep='\t')
	df2 = pd.read_csv('stereo_dataset/'+engine2+'_data_fr.csv', encoding='utf-8', sep='\t')
	df3 = pd.read_csv('stereo_dataset/'+engine3+'_data_fr.csv', encoding='utf-8', sep='\t')
	df4 = pd.read_csv('stereo_dataset/'+engine4+'_data_fr.csv', encoding='utf-8', sep='\t')
