...
def comparison_across_models(cat_of_interest, language, lan_nrc):

    models =[language+'_emotion_bert-base-multilingual-uncased', language+'_emotion_xlm-roberta-base', language+'_emotion_xlm-roberta-large']
    labels = ['mBERT', 'XLMR-B', 'XLMR-L']
    output_dir = 'mlm_output/'    
    shifts, vectors = [], []
    for i in range(0, len(models)):  
        array, targets = compute_emotion_scores(models[i],  cat_of_interest,  output_dir, labels[i], language, lan_nrc)
