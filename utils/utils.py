...
def get_mlm_output(Model, topk, Group, language):
    names = {'mBERT': 'bert-base-multilingual-uncased', 'XLMR-B': 'xlm-roberta-base', 'XLMR-L': 'xlm-roberta-large'}
    original_ranked=json.load(open('./mlm_output/'+names[Model]+'_'+language+'.json',"r")) 
    reranked=json.load(open('./mlm_output/'+names[Model]+'_'+language+'-corrected.json',"r")) 
    templates = []
...
def search_engine_stereotypes(names, dataset, word_for_so):
    get_stereotypes_per_tgt(names, dataset, word_for_so)

def get_stereotypes_per_tgt(names, dataset='./data_collection/stereo_dataset/single_word_stereo_dataset.csv', word_for_so='so'):
    '''
    ...
    for index, row in df.iterrows():
        ...
        if inputs[-1] != word_for_so:
            ind = inputs.index(word_for_so)
